...
balt.mort.nat <- 1*(cbind(balt.mort.nat.init, Sprat, Herring, Cod) > 0)
...
wss.mort.nat <- 1*(cbind(wss.mort.nat.init, Cod, Silver.Hake, Halibut, Pollock, D.Piscivores, L.Benthivores, Skates, 
                      Redfish, A.Plaice, Flounders, Haddock, L.Sculpin, Herring) > 0)
...
ess.mort.nat <- 1*(cbind(ess.mort.nat.init, Cod, Silver.Hake, Dem.Piscivores, Dem.Feeders, 
                      Halibut, Crabs) > 0)
...
bs.mort.nat <- 1*(cbind(bs.mort.nat.init, Herring, Capelin, Cod, Haddock, Saithe) > 0)
...
goa.mort.nat <- 1*(cbind(goa.mort.nat.init, Pollock, Cod, Herring, Arrowtooth, Halibut, Flathead,
                      Sablefish.comb, Perch, Salmon, Mackerel, Shortspine) > 0)
...
hs.mort.nat <- 1*(cbind(hs.mort.nat.init, Pollock.comb, Herring, Halibut, Cod, Sablefish, Lingcod,
                     Turbot, Flatfish, Pisc.rockfish, Plank.rockfish) > 0)
...
Saithe <- combine.groups(c('Juvenile Saithe (0-3, 0-40cm)', 'Saithe (adult)'), nort.mort.nat.init,
                         group.info)
nort.mort.nat <- 1*(cbind(nort.mort.nat.init, Herring, Whiting, Haddock, Saithe, Cod) > 0)
...
group.info <- read.group.info(ecosystem=ecosystem)
gom.mort.nat <- 1*(gom.diet*
                     t(aaply(.data=group.info, .margins=1, .fun=expand.group, df=group.info, .expand=FALSE))
                   > 0)
...
ebs.mort.nat <- 1*(cbind(ebs.mort.nat.init, Pollock, Cod, all.herring, Arrowtooth, Kamchatka, Turbot,
                      Halibut, Yellowfin, Flathead, Rock.sole, all.sablefish, Mackerel, Salmon) > 0)
...
save('pred.ts.data', 'all.regions.byspp', 'mort.props.joined', file = '~/Dropbox/chapter2/R/predation_dfs_binary.RData')
